from flask import Blueprint, render_template, send_from_directory, g, request, session, jsonify
...
def index():
    ...
    # Always use the original landing page template for now
...
from flask import Blueprint, render_template, send_from_directory, g, request, jsonify
import json
import os
...
@main_blueprint.route('/application_groups/<section>/application_group_root/static/data/<filename>')
def serve_program_json(section, filename):
    try:
        # Use the path structure that worked in your debug output
        json_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            'transaction_platform_app',
            'app',
            'application_groups',
            section,
        ...
        # Modify the title based on customer config
        if section == 'houseapps':  # Only modify for houseapps section
            print(f"Debug - Current config: {g.config}")
            # Use the same applications_dock_text that works in the sidebar
            dock_title = g.config.get('menu_config', {}).get('applications_dock_text', 'House Applications Dock')
            data['title'] = dock_title
            print(f"Debug - Set title to: {dock_title}")
        
        return jsonify(data)
        
    except Exception as e:
        print(f"Error serving program JSON: {str(e)}")
        print(f"Attempted path was: {json_path}")
        return jsonify({'error': 'Error loading program data'}), 500
