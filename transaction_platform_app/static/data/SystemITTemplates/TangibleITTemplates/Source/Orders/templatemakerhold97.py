...
from variables_config import exact_replacements, wildcard_replacements, cleanup_replacements
...
def process_template(file_path):
    ...
    try:
        ...
        # Phase 1: Process exact replacements first
        print("\nPhase 1: Processing exact replacements...")
        ...
        # Phase 2: Run wildcard cleanup operations
        print("\nPhase 2: Running wildcard cleanup operations...")
        for find_text, replace_text in wildcard_replacements.items():
            print(f"\nProcessing wildcard cleanup: {find_text}")
            safe_find_replace(doc, find_text, replace_text, use_wildcards=True)
        
        # Phase 3: Final cleanup (if needed)
        print("\nPhase 3: Final cleanup...")
        if hasattr(cleanup_replacements, 'items'):
            for find_text, replace_text in cleanup_replacements.items():
                print(f"\nFinal cleanup: {find_text}")
                safe_find_replace(doc, find_text, replace_text, use_wildcards=False)
        ...
        final_path = Path(script_dir) / f"{original_filename}_processed.docx"
        print(f"Moving file to script directory: {final_path}")
