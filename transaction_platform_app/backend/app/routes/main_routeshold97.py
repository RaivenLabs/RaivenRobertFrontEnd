...
@main_blueprint.route('/api/templates/<path:template_path>', methods=['GET'])
def get_template_content(template_path):
    """Route for getting template content"""
    try:
        # Construct full path from the template path
        base_path = Path(current_app.root_path).parent.parent / 'static' / 'data'
        ...
        with open(full_path, 'rb') as f:  # Note the 'rb' for binary reading
            content = f.read()
...
def convert_template():
    """
    API endpoint to initiate template conversion process
    """
    data = request.get_json()
    ...
    # Validate required template information
    if not template_info.get('sourceTemplatePath'):
        return jsonify({
            'success': False,
            'error': 'Missing source template path'
        }), 400
    
    # Get the relative path from the template info
    source_template_path = template_info['sourceTemplatePath']
    
    # Handle path transformation based on environment/format
    if source_template_path.startswith('/static'):
        # Convert relative path to absolute using your established pattern
        absolute_path = os.path.abspath(os.path.join(
            current_app.root_path,
            '..',
            '..',
            source_template_path.lstrip('/')
        ))
        print(f"\n🔍 Converting relative path to: {absolute_path}")
    else:
        # Path is already absolute
        absolute_path = source_template_path
    
    # Ensure the path is valid and file exists
    if not os.path.exists(absolute_path):
        print(f"\n⚠️ Template file not found: {absolute_path}")
        return jsonify({
            'success': False,
            'error': f'Template file not found: {absolute_path}'
        }), 404
    
    print(f"\n✅ Found template file at: {absolute_path}")
    ...
    conversion_thread = Thread(
        ...
        args=(job_id, absolute_path, template_info, status_dir)
    )
    conversion_thread.daemon = True  # Make thread exit when main thread exits
...
def run_conversion_process(job_id, source_template_path, template_info, status_dir):
    """Run the template conversion process in the background"""
    try:
        ...
        update_conversion_status(job_id, 'starting', 'Initializing conversion process', status_dir)
        
        # Prepare conversion parameters
        template_type = template_info.get('templateType', 'standard')
        template_name = os.path.basename(source_template_path)
        
        # Log the conversion start
        update_conversion_status(job_id, 'converting', f'Starting conversion of {template_name}', status_dir)
        
        # Use the imported template_converter
        try:
            output_file = template_converter.convert_template(source_template_path)
        ...
        update_conversion_status(job_id, 'error', f'Exception during conversion: {str(e)}', status_dir)
    finally:
...
def read_status_file(job_id, status_dir):
...
def update_conversion_status(job_id, status, message, status_dir, additional_data=None):
    ...
    existing_data['last_message'] = message
