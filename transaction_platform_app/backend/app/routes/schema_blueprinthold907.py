...
def analyze_documents():
    ...
    # Try to load briefing context (both context.yaml and scope.txt) for more information
    try:
...
@schema_blueprint.route('/test-claude-connection', methods=['GET'])
def test_claude_connection():
    """Simple endpoint to test if we can connect to Claude API"""
    try:
        # Call Claude API with a simple message
        response = anthropic_client.messages.create(
            model='claude-3-7-sonnet-20250219',
            max_tokens=100,
            temperature=0,
            messages=[
                {"role": "user", "content": "Please respond with 'Connection successful' and confirm you're Claude 3.7 Sonnet."}
            ]
        )
        
        # Get the response content - extract the text properly
        response_text = response.content[0].text if response.content and len(response.content) > 0 else "No response"
        
        # Log success for debugging
        current_app.logger.info(f"Successfully connected to Claude API. Response: {response_text}")
        ...
        current_app.logger.error(f"Error connecting to Claude API: {str(e)}")
        return jsonify({
            "status": "error",
...
def build_schema_generation_prompt(analysis_results, context):
    ...
    # Get context and scope from the new structure
    yaml_context = briefing_context.get('context', {}) if briefing_context else {}
    business_context = yaml_context.get('business_context', '') if yaml_context else ''
    
    # Get scope information
    scope_text = briefing_context.get('scope', '') if briefing_context else ''
    ...
    Based on the analysis of these documents and the provided context and scope, create a comprehensive schema that captures all relevant information.
...
def get_schema(project_id):
    ...
    try:
        ...
        if is_development and os.environ.get('USE_LOCAL_FILES', 'False').lower() == 'true':
            ...
            # For debugging
            current_app.logger.info(f"Attempting to read schema from path: {schema_path}")
            ...
            try:
                ...
                return jsonify({"error": f"Schema not found: {str(e)}"}), 404
