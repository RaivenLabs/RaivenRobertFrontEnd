...
// Add init method
init() {
    console.log('🚀 Initializing Ready Room handler...');
    // Remove URL check - if we're initializing, we want the guide
    this.loadReadyRoomGuide().catch(error => {
    ...
    setupEventListeners() {
        document.addEventListener('click', async (e) => {
            ...
            console.log(`🎯 Handling Ready Room navigation: ${section}`);

            switch (section) {
                case 'readyroomtour':
                    console.log('📚 Loading Application Guide...');
                    await this.loadReadyRoomGuide();
                    break;
                ...
                case 'launchdealmaker':
                 ...
                 // In readyroom_handler.js, modify the switch case to:
                case 'readyroomportfolio':
                    console.log('📋 Loading Transactions In Flight...');
                    await this.loadTransactionsInflight();  
                    break;   

                case 'readyroomfeaturerequests':
                    console.log('📋 Loading Feature Requests...');
                    await this.loadReadyRoomFeatureRequestDesk();
                    break;  
    ...
    setupConsoleListeners() {
        document.addEventListener('click', async (e) => {
            if (e.target.closest('.application-console')) {
                const readyRoomLaunchButton = e.target.closest('.readyroom-launch-btn');
                const closeButton = e.target.closest('.close-button');
                
                if (closeButton) {
                    this.closeApplicationConsole();
                } else if (readyRoomLaunchButton) {
                    console.log('🚀 Ready Room Launch Button clicked');
                    const group = readyRoomLaunchButton.closest('.program-group');
                    ...
                    if (selectedProgram) {
                        ...
                        try {
                            ...
                            appElement.setAttribute('data-origin-section', 'readyroom');
                            ...
                            const { handler } = await import(`/application_groups/readyroom/applications/${programID}/static/handlers/application_handler.js`);
    ...
    async loadReadyRoomGuide() {
        try {
            console.log('📖 Importing guide module...');
            const { program } = await import('/application_groups/readyroom/applications/readyroom/static/js/guide.js');
    ...
    async loadTransactionsInflight() {
        try {
            ...
            const { program } = await import('/application_groups/readyroom/applications/readyroom/static/js/data_table.js');
            console.log('🎯 Launching Ready Room dashboard...');
    
            // Load and initialize the dashboard in the engagement window
            await program.launch('readyroom-dashboard', document.querySelector('.engagement-window'));
            console.log('✅ Ready Room dashboard loaded successfully');
            
        } catch (error) {
            console.error('❌ Error loading Ready Room dashboard:', error);
            this.showComingSoon('Ready Room Dashboard'); 
        }
    },
    ...
    async launchCloser() {
        try {
            console.log('🔄 Importing Ready Room cycle module...');
            const { program } = await import('/application_groups/readyroom/applications/readyroom/static/js/cycle.js');
            console.log('🎯 Launching Ready Room cycle module...');
            await program.launch('cycle', document.querySelector('.engagement-window'));
            console.log('✅ Ready Room engineer loaded successfully');
        } catch (error) {
            console.error('❌ Error loading Ready Room cycle:', error);
            this.showComingSoon('Ready Room Engineer');
        }
    },
    ...
    async loadBuildKits() {
        try {
            console.log('📋 Starting to load Ready Room Package ...');
            
            console.log('Making fetch request to:', '/application_groups/readyroom/application_group_root/static/data/readyroom_programs.json');
            const response = await fetch('/application_groups/readyroom/application_group_root/static/data/readyroom_programs.json');
            ...
            // Change these lines to use the Ready Room functions
            this.launchDealmaker(data);
            this.setupConsoleListeners();
            console.log('✅ Ready Room Package loaded successfully');
        } catch (error) {
            console.error('❌ Detailed error loading Ready Room Package:', error);
            console.error('Error stack:', error.stack);
            this.showComingSoon('Ready Room Package');
        }
    },

    launchDealmaker(data) {

        const customerTitle = this.currentSection === 'readyroom' 
        ...
        return groups.map(group => `
            <div class="program-group">
                <div class="group-header">
                    ...
                    <button class="btn btn-primary readyroom-launch-btn">Launch Program</button>
                </div>
    ...
    async launchDealmakerProgram() {
        ...
        const programID = selectedProgram.value;
        console.log(`Launching ${programID} from readyroom section`);
        
        try {
            // Close console before attempting to load handler
            this.closeApplicationConsole();
    
            const handlerPath = `/application_groups/readyroom/applications/${programID}/static/handlers/application_handler.js`;
            ...
            if (handler && typeof handler.initialize === 'function') {
                ...
                await handler.initialize('readyroom', 'application');
            } else {
                console.log(`No handler found for ${programID}, falling back to default route`);
                const route = `/${programID.replace('-', '_')}?section=readyroom`;
    ...
    async loadReadyRoomFeatureRequestDesk() {
        try {
            console.log('📋 Importing Support Desk Module...');
            const { program } = await import('/application_groups/readyroom/applications/readyroom/static/js/support_desk.js');
...
console.log('📦 Ready Room handler module loaded');
handler.init();
