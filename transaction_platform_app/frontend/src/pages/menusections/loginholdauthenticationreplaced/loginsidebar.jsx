...
import { speakeasyConfig } from '../../../config/sectionNavigation';
...
let loginApplicationsConfig;
try {
  loginApplicationsConfig = require('../../../config/applicationNavigation/loginApplications');
} catch {
  console.log('📝 No applications config found for this section');
}

const LoginSidebar = ({ onSidebarChange }) => {
  ...
  const hasApplicationItems = (sectionId) => {
    return loginApplicationsConfig?.applicationItems?.length > 0 && 
           sectionId === 'speakeasy-applications';
  };
    ...
    <>
      <div className="flex flex-col h-full bg-sidebarDark text-ivory shadow-sidebar relative">
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Section menu items */}
          {speakeasyConfig.sectionItems.map((item) => (
            <div key={item.id}>
              <button
                onClick={() => {
              ...
              {hasApplicationItems(item.id) && expandedSection === item.id && 
                loginApplicationsConfig.applicationItems.map((appItem) => {
