...
import { settingsConfig } from '../../../config/sectionNavigation';
...
let settingsApplicationsConfig;
try {
  settingsApplicationsConfig = require('../../../config/applicationNavigation/settingsApplications/config');
} catch {
  console.log('📝 No applications config found for this section');
}

const SettingsSidebar = ({ onSidebarChange }) => {
  ...
  const handleNavigation = async (item) => {
    ...
    if (item.id === 'settings-applications') {
  ...
  const hasApplicationItems = (sectionId) => {
    return settingsApplicationsConfig?.applicationItems?.length > 0 && 
           sectionId === 'settings-applications';
  };
    ...
    <>
      <div className="flex flex-col h-full bg-sidebarDark text-ivory shadow-sidebar relative">
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Section menu items */}
          {settingsConfig.sectionItems.map((item) => (
            <div key={item.id}>
              <button
                onClick={() => {
              ...
              {hasApplicationItems(item.id) && expandedSection === item.id && 
                settingsApplicationsConfig.applicationItems.map((appItem) => {
