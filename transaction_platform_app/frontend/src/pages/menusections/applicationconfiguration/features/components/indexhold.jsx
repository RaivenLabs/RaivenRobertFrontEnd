...
const ConfigurableLanding = () => {
  ...
  const sourcingPrograms = [
    ...
    },
    {
      ...
      description: 'Standard purchase order agreements'
    },
    {
      id: 'msa',
      name: 'MSA',
    ...
    },
    {
      id: 'consulting',
      name: 'Consulting',
      description: 'Consulting service agreements'
    },
    {
      id: 'hardware',
      name: 'Hardware',
      description: 'Hardware purchase agreements'
    }
  ];
  ...
  const renderContent = () => {
    ...
    if (currentStage !== TEMPLATE_STAGES.PROGRAM_SELECTION) {
      return (
        <TemplateConfig 
        ...
        <div className="flex flex-col h-full bg-white rounded-xl shadow-lg border border-gray-100">
          ...
          <div className="p-6 space-y-4">
            {!selectedGroup || !templateFoundation ? (
              <div className="text-center py-8 text-gray-500">
                Select program group and foundation first
              </div>
            ) : (
              <>
                {sourcingPrograms.map((program) => (
                  <button
                    key={program.id}
                    onClick={() => setProgramClass(program.id)}
                    className={`w-full p-4 rounded-lg border transition-all
                      ${programClass === program.id
                        ? 'border-teal bg-teal/5'
                        : 'border-gray-200 hover:border-teal/50'
                      }`}
                  >
  ...
  return (
    <div className="min-h-screen bg-ivory p-6 space-y-6">
      ...
      {/* Action Panel - Only show for initial selection stage */}
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              disabled={!selectedGroup || !templateFoundation || !programClass || isTransitioning}
              className={`px-6 py-2 rounded-lg transition-colors
                flex items-center gap-2
                ${selectedGroup && templateFoundation && programClass && !isTransitioning
