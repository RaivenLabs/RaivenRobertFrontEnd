...
const FundLaunchPanel = ({ onComplete, disabled }) => {
  ...
  // Utility function for safe nested value access
  ...
  useEffect(() => {
    if (!isStepCompleted && selectedFunds.length > 0 && isConfirmed) {
      ...
      setProject({
        ...
        type: 'fund_analysis',
        selectedFunds: selectedFunds.map(frn => ({
          frn,
          name: getNestedValue(registryData, `firms.${frn}.name`, 'Unknown Fund'),
          type: getNestedValue(registryData, `firms.${frn}.type`, 'Unknown Type')
        }))
      });
      
      onComplete();
    }
  }, [selectedFunds, isConfirmed, isStepCompleted, onComplete, setProject, registryData]);
  ...
  const handleConfirm = () => {
    ...
    console.log('Raw selected FRNs:', selectedFunds);
    
    // Format and store the funds in context
    const formattedFunds = selectedFunds.map(frn => ({
      frn: frn,  // The key identifier we'll need later
      name: getNestedValue(registryData, `firms.${frn}.name`, 'Unknown Fund'),
      type: getNestedValue(registryData, `firms.${frn}.type`, 'Unknown Type'),
      registryData: registryData.firms[frn]  // Store full registry data for this fund
    }));
    
    console.log('📦 Formatted funds before adding to context:', formattedFunds);
    
    addSelectedFunds(formattedFunds);
    
    // Let's verify what's in context right after adding
    console.log('🗃️ Current context after adding funds:', {
      contextFunds: selectedFunds,  // This comes from useFundContext()
  ...
  const filteredFunds = registryData?.firms 
    ? Object.entries(registryData.firms).filter(([frn, fund]) => {
        const name = getNestedValue(fund, 'name', '').toLowerCase();
        const type = getNestedValue(fund, 'type', '').toLowerCase();
        const searchLower = searchTerm.toLowerCase();
        
        return name.includes(searchLower) ||
               type.includes(searchLower) ||
               frn.includes(searchLower);
      })
    : [];
                ...
                <label className="flex items-start space-x-3 cursor-pointer">
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">
                      {getNestedValue(fund, 'name', 'Unnamed Fund')}
                    </div>
                    <div className="text-sm text-gray-500 mt-1">
                      <span className="mr-3">FRN: {frn}</span>
                      <span className="mr-3">
                        Type: {getNestedValue(fund, 'type', 'Unknown Type')}
                      </span>
                      <span>
                        Status: {getNestedValue(fund, 'status', 'Unknown Status')}
                      </span>
                    </div>
                  </div>
                </label>
            ...
            <div className="space-y-2">
              {selectedFunds.map(frn => (
                <div key={frn} className="text-sm text-gray-600">
                  {getNestedValue(registryData, `firms.${frn}.name`, 'Unknown Fund')} (FRN: {frn})
                </div>
              ))}
            </div>
          </div>
