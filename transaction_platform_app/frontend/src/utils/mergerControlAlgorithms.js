...
export const THRESHOLDS = {
  ...
  export function validateCompanyData(companyData) {
    const requiredFields = [
      'revenue.numeric.global',
      'revenue.numeric.us',
      'revenue.numeric.eu',
      'assets.numeric.global',
      'size_of_person.annual_net_sales.numeric',
      'size_of_person.total_assets.numeric',
      'is_manufacturer'
    ];
  
    const missingFields = requiredFields.filter(field => {
      const value = field.split('.').reduce((obj, key) => obj?.[key], companyData);
