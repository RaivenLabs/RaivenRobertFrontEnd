...
import { r2d2Config } from '../../../config/sectionNavigation';
import { ArrowLeft } from 'lucide-react';

const R2D2Sidebar = ({ onSidebarChange }) => {
  const navigate = useNavigate();
  const [sectionItems, setSectionItems] = useState([]);

  useEffect(() => {
    console.log('Initial r2d2Config:', r2d2Config);
    
    // Safely set section items
    if (r2d2Config?.sectionItems) {
      console.log('Setting section items:', r2d2Config.sectionItems);
      setSectionItems(r2d2Config.sectionItems);
    } else {
      console.warn('No section items found in r2d2Config');
    }
  }, []);
