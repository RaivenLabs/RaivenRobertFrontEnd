...
import { buildkitsConfig } from '../../../config/sectionNavigation';
import { ArrowLeft } from 'lucide-react';

const BuildKitsSidebar = ({ onSidebarChange }) => {
  const navigate = useNavigate();
  const [sectionItems, setSectionItems] = useState([]);

  useEffect(() => {
    console.log('Initial buildkitsConfig:', buildkitsConfig);
    
    // Safely set section items
    if (buildkitsConfig?.sectionItems) {
      console.log('Setting section items:', buildkitsConfig.sectionItems);
      setSectionItems(buildkitsConfig.sectionItems);
    } else {
      console.warn('No section items found in buildkitsConfig');
    }
  }, []);
...
export default BuildKitsSidebar;
